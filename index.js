...
async function initializeBot() {
    try {
        console.log('Attempting to connect to MongoDB with URI:', MONGODB_URI.replace(/\/\/.*@/, '//<hidden>@'));
        await mongoose.connect(MONGODB_URI, mongooseOptions);
        ...
        console.log('Webhook callback configured');

        // Health check endpoint
        app.get('/', (req, res) => {
            res.status(200).send('EV India Bot server is running and DB connected!');
        });
