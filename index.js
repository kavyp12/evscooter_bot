...
const mongooseOptions = {
    ...
    family: 4 // Use IPv4, skip trying IPv6 if causing issues
};

// --- Define Mongoose Schemas and Models ---
// (Your schemas: scooterSchema, dealerSchema, conversationSchema remain the same)
...
mongoose.connection.on('error', (err) => {
    ...
    // Consider exiting if critical, or implementing retry logic
...
// Initialize bot instance (declare here, configure after DB connection)
const bot = new Telegraf(TELEGRAM_BOT_TOKEN);

// --- Helper Functions ---
// (Your helper functions: queryChatGPT, saveConversation, getConversationHistory,
// checkAvailabilityByPincode, getScooterInfo, getComparisonInfo, extractPincode,
// extractPotentialModels - remain largely the same. Ensure getScooterInfo has the regex fix)
...
async function saveConversation(ctx, userMessage, botMessage) {
    if (!ctx.from || !ctx.from.id) {
        console.error('Cannot save conversation: ctx.from.id is missing.');
        return;
    }
    ...
    try {
        await Conversation.findOneAndUpdate(
            { userId: userId },
            {
                ...
                $set: { lastInteraction: new Date(), telegramChatId: telegramChatId }
            },
...
async function getConversationHistory(userId) {
    try {
        ...
        const recentConversations = record.conversations.slice(-10); // Last 5 exchanges
...
async function getScooterInfo(modelName) { // Ensure this has the $options fix
    try {
        const scooter = await Scooter.findOne({ model: { $regex: new RegExp(`^${modelName}$`, 'i') } });

        if (!scooter) {
            const similarScooters = await Scooter.find({ model: { $regex: modelName, $options: 'i' } }).limit(1); // CORRECTED REGEX
            ...
            // ... (rest of the responseText generation for aSimilarScooter as in the previous corrected version)
        ...
        // ... (rest of the responseText generation for exact match as before)
...
async function getComparisonInfo(modelName1, modelName2) {
    try {
        // Fetch details for both models (make sure getScooterInfo is robust)
...
async function extractPotentialModels(message) {
    try {
        const allScooters = await Scooter.find({}, 'model brand').lean(); // .lean() for performance if not modifying
        const models = [];
...
// (Your bot handlers: bot.start, bot.help, bot.on('text') remain the same)
bot.start(async (ctx) => {
...
How can I assist you today?`;
    try {
        await ctx.reply(welcomeMessage, Markup.keyboard([
            ...
            // ['Latest EV News (Coming Soon!)', '/help'] // Example for later
            ['/help']
        ]).resize().oneTime());
...
bot.on('text', async (ctx) => {
    ...
    try {
        ...
        const comparisonRegex = /compare ([\w\s.-]+) and ([\w\s.-]+)/i; // Allow hyphens and dots in model names
        ...
        if (pincode) {
            ...
            const potentialModels = await extractPotentialModels(userMessage); // This can return multiple
            if (potentialModels.length === 1) {
                ...
                // If multiple models are vaguely mentioned, could ask user to clarify or pass to GPT
                // For now, let's pass to GPT.
                const conversationHistory = await getConversationHistory(userId);
                botResponse = await queryChatGPT(userMessage, conversationHistory);
                 // botResponse = `I found mentions of a few models: ${potentialModels.join(', ')}. Which one are you specifically interested in?`;
            } else { // General query or model not in DB / not clearly stated
                const conversationHistory = await getConversationHistory(userId);
                botResponse = await queryChatGPT(userMessage, conversationHistory);
            }
        }
        ...
        // Check if the error is a Mongoose timeout error specifically
...
async function seedSampleData() {
    try {
        ...
        await Scooter.deleteMany({}); // Clear before seeding to avoid duplicates if run multiple times with partial data
...
async function main() {
    try {
        ...
        // Connection successful at this point if no error is thrown

        // Initialize session middleware only after successful DB connection
        bot.use(session({
            store: {
                collection: mongoose.connection.collection('sessions'),
            },
            // ttl: 60 * 60 * 24, // 1 day in seconds (optional)
        }));
        console.log('Mongoose session middleware initialized successfully.');

        // Seed data (this will now run after successful connection)
        ...
        // Graceful stop (already handled by SIGINT process listener for Mongoose)
        // Telegraf also has its own stop mechanism tied to process signals
        const stopBot = (signal) => {
            console.log(`${signal} received, stopping bot...`);
            bot.stop(signal);
            // Mongoose connection is closed by its own SIGINT handler
        };
        ...
        process.exit(1); // Exit if critical components fail to start
    }
}
